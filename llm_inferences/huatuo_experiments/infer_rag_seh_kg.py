...
def prompt_creation_rag(symptoms, text,references,possible_diseases,k):
...
def get_model_output(prompt):
    ...
    codes= extract_ranked_icd9_codes(text)
    del generated
    del inputs
...
for i,row in mimic.iterrows():
    try:
        ...
        results = f"""
            ...
            {row.top_2_symptoms}
    """
        possible_diseases=row.predicted_diseases
        predicted_codes = get_model_output(prompt_creation_rag(symptom_list, text, results,possible_diseases,k))
