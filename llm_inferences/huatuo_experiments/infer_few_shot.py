...
def build_prompt(symptoms: str, ehr: str,fewshot_examples:list,k: int) -> str:
    prompt= f"""
...
def predict_icd9(symptoms: str, ehr: str,fewshot_examples,k: int, max_new_tokens: int = 2048):
    prompt = build_prompt(symptoms, ehr,fewshot_examples,k)
...
if __name__ == "__main__":
    ...
    for idx, row in enumerate(df.itertuples(), start=0):
        ...
        true_codes = [c.strip().upper() for c in row.true_disease_codes.split(",") if c.strip()]
        ehr=row.ehr
        # number of disease codes to retreive [5,10,15]
        k = 15
        pred_codes = predict_icd9(row.symptoms,ehr,fewshot_examples,k)
        print(pred_codes)
        results.append({
