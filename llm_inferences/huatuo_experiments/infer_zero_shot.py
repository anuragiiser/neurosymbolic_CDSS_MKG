...
def build_prompt(symptoms: str, ehr: str, k: int) -> str:
    return f"""
...
def predict_icd9(symptoms: str, ehr: str, k: int, max_new_tokens: int = 2048):
    prompt = build_prompt(symptoms, ehr, k)
...
if __name__ == "__main__":
    ...
    for idx, row in enumerate(df.itertuples(), start=0):
        ...
        k = 15
        pred_codes = predict_icd9(row.symptoms,ehr, k)
        print(pred_codes)
        results.append({
