...
def build_prompt(symptoms: str,ehr,fewshot_examples,ret_diseases,k:int) -> str:
    prompt=f"""
    ...
    for ex in fewshot_examples:
        prompt += f"""
            ...
            ehr: {ex['ehr']}
            "retrieved possible diseases": {str(ex.get("predicted_diseases", ""))},
...
def predict_icd9(symptoms: str, ehr,few_shot,ret_diseases,k: int=15):
    messages = [
    {
        "role": "system",
    ...
    {
        "role": "user",
        "content": build_prompt(symptoms,ehr,few_shot,ret_diseases,k)
    }
    ]
...
for i, row in df.iterrows():
    try:
        ...
        true_codes = [c.strip().upper() for c in str(row["short_codes"]).split(",") if c.strip()]
        ehr=row['ehr']
        k=15
        ret_diseases=row.predicted_diseases
        pred_codes=predict_icd9(symptoms,ehr,fewshot_examples,ret_diseases,k=15)
        print(pred_codes)
        results.append({
            "symptoms": symptoms,
