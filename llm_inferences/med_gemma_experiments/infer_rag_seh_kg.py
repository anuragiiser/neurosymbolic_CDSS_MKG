...
def prompt_creation_rag(symptoms, text, references,ret_diseases,k):
...
- You are also provided with list of possible diseases, which you can use as hints in predicting the codes, but you should not be limited by it.
...
Possible Diseases (use as hints):
{ret_diseases}
...
def extract_ranked_icd9_codes(response_text):
    ...
    for code in codes:
        ...
        if cd[0]=='V' or cd[0]=='E':
            cd = cd.replace('.', '')
            code_set.append(f"{rank}:{cd[:4]}")
        else:
            code_set.append(f"{rank}:{cd[:3]}")
    return code_set


def get_model_output(symptoms, text, references, ret_diseases, k):
    messages = [
    {
        "role": "system",
    ...
    {
        "role": "user",
        "content": prompt_creation_rag(symptoms,text,references,ret_diseases,k)
    }
    ]
...
for i,row in mimic.iterrows():
    ...
    """
        ...
        if i==0:
            print(prompt_creation_rag(symptom_list,text,results,ret_diseases,k))
        predicted_codes = get_model_output(symptom_list, text, results,ret_diseases,k)
